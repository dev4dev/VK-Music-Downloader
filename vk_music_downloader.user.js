...
var icon_download = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAGXRFWHRTb2Z0d2FyZQBBZG9iZSBJ\
...
function get_info(id){
	var obj = document.getElementById('audio_info' + id);
	if (obj) {
		var val = obj.getAttribute('value').split(',');
		$url = val[0];
		$length = val[1];
		var next = obj.parentNode.nextSibling.nextSibling;
		var ttl = document.getElementById('title' + id);
		if (!ttl) {
			var ttl = document.getElementById('audio' + id).getElementsByClassName('title')[0];
		};
		$title = ttl.innerHTML.replace(/<[^>]+>/g,"").replace('amp;', '');
		var artist = next.getElementsByTagName('b')[0];
		$artist = artist.innerHTML.replace(/<[^>]+>/g,"").replace('amp;', '');
		var $dur = next.getElementsByClassName('duration')[0];
		return {
			'target': $dur,
			'artist': $artist,
			'title' : $title,
